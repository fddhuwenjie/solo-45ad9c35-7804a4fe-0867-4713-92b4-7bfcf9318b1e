...
from .processing.units import (
    FLOW_TO_M3H, FLOW_TO_NL_MIN, PRESSURE_TO_KPA, TEMP_OFFSET_K,
    TRAVEL_UNITS)
...
# 标准状态气体流量
FLOW_GAS_UNITS = tuple(FLOW_TO_NL_MIN.keys())
# 液体工况体积流量（含 m³/h 等既有分析已支持的 Unicode 写法）
FLOW_LIQUID_UNITS = tuple(FLOW_TO_M3H.keys())
...
_FACTORS = {
    ...
    "flow_gas": {u: f for u, f in FLOW_TO_NL_MIN.items()},
    # 保留 m³/h、m³/min、l/min、l/h 等 Unicode 等价写法（与流量曲线分析一致）
    "flow_liquid": {u: f for u, f in FLOW_TO_M3H.items()},
}
...
def _to_celsius(value, u):
    ...
    if u == "c":
        return float(value)
    if u == "k":
        return float(value) - TEMP_OFFSET_K
    if u == "f":
