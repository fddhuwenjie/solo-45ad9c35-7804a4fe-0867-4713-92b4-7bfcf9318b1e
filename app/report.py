...
def _metrics_table(result):
    ...
    m = result["metrics"]
    rows = [
        ("行程时间 (s)", m["travel_time"]["max_s"], thr["travel_time_s_max"]),
        ("死区 (%)", m["deadband"]["max_pct"], thr["deadband_pct_max"]),
        ("回差 (%)", m["hysteresis"]["max_pct"], thr["hysteresis_pct_max"]),
        ("过冲 (%)", m["overshoot"]["max_pct"], thr["overshoot_pct_max"]),
        ("稳态偏差 (%)", m["steady_state"]["max_pct"], thr["steady_state_pct_max"]),
    ]
    out = ["<table><thead><tr><th>指标</th><th>数值</th><th>阈值</th><th>判定</th></tr></thead><tbody>"]
    for name, val, limit in rows:
        if val is None:
            out.append(f'<tr><td>{name}</td><td colspan="2">—</td><td class="warn">无数据</td></tr>')
            continue
        ok = val <= limit
        cls = "ok" if ok else "bad"
        out.append(f'<tr><td>{name}</td><td>{val}</td><td>≤ {limit}</td>'
                   f'<td class="{cls}">{"合格" if ok else "超限"}</td></tr>')
