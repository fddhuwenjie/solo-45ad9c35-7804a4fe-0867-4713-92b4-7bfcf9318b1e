...
CREATE TABLE IF NOT EXISTS limitswitch_tests (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  valve_id INTEGER NOT NULL REFERENCES valves(id),
  phase TEXT NOT NULL,
  submitted_at TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS limitswitch_analyses (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  ls_test_id INTEGER NOT NULL REFERENCES limitswitch_tests(id),
  version INTEGER NOT NULL,
  created_at TEXT NOT NULL,
  author TEXT NOT NULL DEFAULT 'auto',
  adjustments_json TEXT NOT NULL DEFAULT '[]',
  result_json TEXT NOT NULL,
  UNIQUE(ls_test_id, version)
);
CREATE TABLE IF NOT EXISTS limitswitch_comparisons (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  valve_id INTEGER,
  created_at TEXT NOT NULL,
  result_json TEXT NOT NULL
...
class Database:
    ...
    # ---- 限位开关（开/关到位接点）诊断 ----
    def create_limitswitch_test(self, valve_id, phase, test_started_at,
                                range_min, range_max, range_unit,
                                conditions, thresholds,
                                calibration_valid_until, payload):
        with self._lock, self._conn:
            cur = self._conn.execute(
                """INSERT INTO limitswitch_tests(valve_id, phase, submitted_at,
                   test_started_at, range_min, range_max, range_unit,
                   conditions_json, thresholds_json, calibration_valid_until,
                   payload_json)
                   VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                (valve_id, phase, _now(), test_started_at, range_min, range_max,
                 range_unit, json.dumps(conditions), json.dumps(thresholds),
    ...
    def get_limitswitch_test(self, ls_test_id):
        row = self._conn.execute(
            "SELECT * FROM limitswitch_tests WHERE id=?", (ls_test_id,)).fetchone()
        if not row:
            return None
        d = dict(row)
        d["conditions"] = json.loads(d.pop("conditions_json"))
    ...
    def list_limitswitch_tests(self, valve_id=None):
        q = ("SELECT id, valve_id, phase, submitted_at, test_started_at "
             "FROM limitswitch_tests")
        args = ()
        if valve_id is not None:
            q += " WHERE valve_id=?"
            args = (valve_id,)
    ...
    def create_limitswitch_analysis(self, ls_test_id, author, adjustments, result):
        with self._lock, self._conn:
            row = self._conn.execute(
                "SELECT COALESCE(MAX(version),0) AS v FROM limitswitch_analyses "
                "WHERE ls_test_id=?", (ls_test_id,)).fetchone()
            version = row["v"] + 1
            cur = self._conn.execute(
                """INSERT INTO limitswitch_analyses(ls_test_id, version, created_at,
                   author, adjustments_json, result_json) VALUES(?,?,?,?,?,?)""",
                (ls_test_id, version, _now(), author,
                 json.dumps(adjustments), json.dumps(result)))
            return cur.lastrowid, version

    def get_limitswitch_analysis(self, analysis_id):
        row = self._conn.execute(
            "SELECT * FROM limitswitch_analyses WHERE id=?", (analysis_id,)).fetchone()
        return self._ls_analysis_row(row)

    def list_limitswitch_analyses(self, ls_test_id):
        rows = self._conn.execute(
            """SELECT id, ls_test_id, version, created_at, author
               FROM limitswitch_analyses WHERE ls_test_id=? ORDER BY version""",
            (ls_test_id,)).fetchall()
        return [dict(r) for r in rows]

    def latest_limitswitch_analysis_ids(self, valve_id):
        """同阀门每个限位开关测试的最新分析版本，按测试时间排序。"""
        rows = self._conn.execute(
            """SELECT t.id AS tid,
                      COALESCE(t.test_started_at, t.submitted_at) AS ttime,
                      (SELECT a.id FROM limitswitch_analyses a
                       WHERE a.ls_test_id=t.id ORDER BY a.version DESC LIMIT 1) AS aid
               FROM limitswitch_tests t WHERE t.valve_id=?
               ORDER BY ttime, t.id""", (valve_id,)).fetchall()
        return [(r["tid"], r["aid"], r["ttime"]) for r in rows if r["aid"] is not None]

    @staticmethod
    def _ls_analysis_row(row):
        if not row:
            return None
        d = dict(row)
        d["adjustments"] = json.loads(d.pop("adjustments_json"))
    ...
    def create_limitswitch_comparison(self, valve_id, result):
        with self._lock, self._conn:
            cur = self._conn.execute(
                """INSERT INTO limitswitch_comparisons(valve_id, created_at, result_json)
                   VALUES(?,?,?)""", (valve_id, _now(), json.dumps(result)))
            return cur.lastrowid
