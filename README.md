...
## 气动执行机构供气瞬态核算（/airsupply）

控制阀在检修台架上动作正常，接到现场仪表风后却可能因细长支管、调压阀流量
不足或多台设备同时耗气而减速；储气罐压力够高，也不代表失气时能走完整个
安全行程。本模块按**方案**做集总参数瞬态核算：方案记录执行器两腔容积、
弹簧曲线、供气管段、调压阀流量特性、储气罐、初始压力及并发用气事件，
提交即冻结；求解器按时间步推进罐压、管段压降、腔压、阀位与瞬时耗气量，
返回各动作的最低供压、预计行程时间与余量。

- 拓扑：气源总管（恒压）→ 供气管段（串联 Darcy-Weisbach 压降）→ 支管节点
  （并发用气事件在此耗气）→ 调压阀（压差-流量曲线，出口受设定压力限制）→
  供气母管节点（储气罐挂接，罐压即供压）→ 腔口 → 被驱动腔；对侧腔直通大气。
  失气安全行程时总管隔离，仅靠储气罐驱动（双作用/弹簧助力）；**弹簧复位
  （spring_return）执行器的失气行程由弹簧驱动**，供气腔经腔口放空，行程
  速度受排气能力限制（仅 A 腔供气的 fail_close 方案可正常提交并核算）。
- 流量曲线覆盖不足、单位冲突、事件倒序或积分不收敛时只列证据缺口
  （`no_conclusion`）。积分收敛性按 dt 与 dt/2 两遍比较行程时间、最低供压
  与终位，**严格遵守 `convergence_tol_pct`**（容差为 0.000001 时只要存在
  相对偏差即判不收敛）。
- 压力低于要求、行程超时或安全位未到达时判 `fail`，并指出首个相关用气
  设备与时间区间（区间内无并发事件时说明瓶颈在供气链）。
- 人工改动并发关系或采用实测边界必须逐条写理由并派生修订；版本比较与
  JSON 导出读取同一冻结方案与逐时结果（不重算）。

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/airsupply/schemes` | 提交核算方案（执行器/管段/调压阀/储气罐/事件/动作，提交即冻结） |
| POST | `/airsupply/schemes/{id}/solve` | 求解当前有效方案，生成修订 |
| POST | `/airsupply/schemes/{id}/revise` | 改动并发关系 / 采用实测边界（须带理由）→ 派生修订 |
| GET | `/airsupply/schemes/{id}/revisions` | 某方案的全部修订 |
| GET | `/airsupply/revisions/{id}` | 读取修订（冻结方案 + 逐时结果） |
| GET | `/airsupply/revisions/{id}/export` | 导出 JSON（同一冻结方案与逐时结果） |
| POST | `/airsupply/comparisons` | 同一方案两修订的版本比较 |
| GET | `/airsupply/comparisons/{id}` | 读取比较记录 |

样例见 `scripts/generate_airsupply_samples.py`（供气充裕、供气不足、
储气罐不足、单位冲突、事件倒序、流量曲线覆盖不足）。
